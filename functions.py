...
def TuneFront(Delta, Factor):
    result = (
        variables.Tune["SpringFrontKgf"] / variables.SpringFrontDelta * Delta
    ) * Factor
    return result
...
def TuneRear(Delta, Factor):
    result = (
        variables.Tune["SpringRearKgf"] / variables.SpringRearDelta * Delta
    ) * Factor
    return result
...
def TuneCar(WeightKg, SpringType, FrontDist, CarClass, DriveType):
    try:
        ...
        if DriveType == "FWD":
            variables.Tune["SpringFrontKgf"] = TuneSpring(
                WeightKgf, FrontDist, SpringType
            ) * (1 - variables.DriveOffset)
            variables.Tune["SpringRearKgf"] = TuneSpring(
                WeightKgf, RearDist, SpringType
            )
        elif DriveType == "RWD":
            variables.Tune["SpringFrontKgf"] = TuneSpring(
                WeightKgf, FrontDist, SpringType
        ...
        if SpringType.upper() == "RALLY" or SpringType.upper() == "STOCK BUGGY":
            variables.Tune["ArbFront"] = (
                TuneFront(constants.Arb_Delta_Front, variables.ArbFactor[CarClass])
                * variables.ArbRallyFactor
            )
            variables.Tune["ArbRear"] = (
                TuneRear(constants.Arb_Delta_Rear, variables.ArbFactor[CarClass])
                * variables.ArbRallyFactor
            )
            variables.Tune["ReboundFront"] = TuneFront(
                constants.Rebound_Delta_Front, variables.ReboundFactor[SpringType]
            )
            variables.Tune["ReboundRear"] = TuneRear(
                constants.Rebound_Delta_Rear, variables.ReboundFactor[SpringType]
            )
            variables.Tune["BumpFront"] = (
                variables.Tune["ReboundFront"] * variables.BumpFactor[SpringType]
            )
            variables.Tune["BumpRear"] = (
                variables.Tune["ReboundRear"] * variables.BumpFactor[SpringType]
            )
        else:
            variables.Tune["ArbFront"] = TuneFront(
                constants.Arb_Delta_Front, variables.ArbFactor[CarClass]
            )
            variables.Tune["ArbRear"] = TuneRear(
                constants.Arb_Delta_Rear, variables.ArbFactor[CarClass]
            )
            variables.Tune["ReboundFront"] = TuneFront(
                constants.Rebound_Delta_Front, variables.ReboundFactor[SpringType]
            )
